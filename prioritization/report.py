...
def export_to_json(
    ...
    for plant in report.plant_scores:
        plant_data = {
            ...
            "scores": {
                "phytochemical": round(plant.component_scores.get("phytochemical", 0.0), 3),
                "ethnobotanical": round(plant.ethnobotanical_score.score, 3),
                "potency": round(plant.component_scores.get("potency", 0.0), 3),
                "resistance": round(plant.component_scores.get("resistance", 0.0), 3),
                "synergy": round(plant.component_scores.get("synergy", 0.0), 3),
                "endophyte": round(plant.component_scores.get("endophyte", 0.0), 3),
            },
            "score_weights": plant.component_weights,
            "metrics": {
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for plant in report.plant_scores:
            row = {
                ...
                'phytochemical_score': round(plant.component_scores.get('phytochemical', 0.0), 3),
                'ethnobotanical_score': round(plant.ethnobotanical_score.score, 3),
                'potency_score': round(plant.component_scores.get('potency', 0.0), 3),
                'resistance_score': round(plant.component_scores.get('resistance', 0.0), 3),
                'synergy_score': round(plant.component_scores.get('synergy', 0.0), 3),
                'endophyte_score': round(plant.component_scores.get('endophyte', 0.0), 3),
                'pathogen_count': plant.pathogen_count,
                'mic_count': plant.mic_count,
                'who_priority_pathogens': plant.who_priority_pathogens,
                'amr_strain_count': plant.amr_strain_count,
